...
func (r *SecSynchReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	//////////////////////	Реализация удаления зависимых объектов с помощью Finalaizer (Начало)
	...
	///////////////////////////////////	Реализация удаления зависимых объектов (Finalaizer) (Конец)
	...
	if sourceSecret.Labels["secsynch-enable"] != "true" {
		...
		if err != nil {
			log.Error(err, "Error adding label to secretr", " Namespace is ", sourceSecret.Namespace, " SecretName is ", sourceSecret.Name)
			return ctrl.Result{}, err
		}
	}
...
func (r *SecSynchReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	predSecret := predicate.Funcs{
		...
		// Ресурс SecSynch пытается копировать секрет, которого нет
